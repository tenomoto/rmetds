...
$\varepsilon_i$は誤差を表しています。 最小分散推定（minimum variance）は最小二乗法（method of least squares）とも呼ばれ、誤差の分散$S=\sum_i \varepsilon_i^2$を最小にする$\beta_0,\,\beta_1$を求めます。

[@eq-lsq.yi]を$\beta_0,\,\beta_1$で微分して0と置くと、正規方程式
...
\begin{aligned}
  ...
  \hat{\beta}_1 & = \frac{\sum(x_i-\bar{y})(y_i-\bar{y})}{\sum(x_i-\bar{x})^2} = \frac{\mathrm{cov}(x,\,y)}{\mathrm{var}(x)}
\end{aligned}
...
y <- c(-0.18, -0.16, -0.2, -0.19, -0.18, -0.21, -0.02, 0.31, 0.32, 0.78, 0.69, 0.4, 0.22, 0.2, -0.14, -0.16, -0.32, -0.4, -0.4, -0.6)
plot(x,y)
abline(lm(y ~ x))
```
